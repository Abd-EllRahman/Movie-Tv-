...
export default function ItemDetails() {
  let { id } = useParams(); // Only fetching ID for TV series
  ...
  // Fetch detailed TV series information by ID
  async function getItemDetails(id) {
    setLoading(true);
    try {
      const response = await axios.get(
        `https://api.themoviedb.org/3/tv/${id}?api_key=44ee5523e457e74020effc2bddc4592e&include_adult=false`
      );
      setItemDetails(response.data);
      await getMovieCast(id); // Fetch the cast after getting series details
      await getSeriesSimilar(id); // Fetch similar series
      setLoading(false);
    } catch (error) {
  ...
  // Fetch cast details for the TV series
  ...
  async function getSeriesSimilar(id) {
    try {
      const similarSeries = await axios.get(
        `https://api.themoviedb.org/3/tv/${id}/recommendations?api_key=44ee5523e457e74020effc2bddc4592e&language=en-US&include_adult=false`
      );
      setSeriesSimilar(similarSeries.data.results); // Update this line to use 'results'
    } catch (error) {
    ...
    <>
      <div className="det">
        ...
        <div className="col-md-9 details-container">
          ...
          <div className="data">
            <h4 className="heading py-2">First Air Date:</h4>
            <p className="ms-3 pt-1 text-light"> {itemDetails?.first_air_date}</p>
          </div>
      ...
      {seriesSimilar.length > 0 && ( // Check if there are similar series
